...
import type { Customer, DiscountType, Product, Quote, QuoteStatus } from "@/lib/types";
...
import { effectiveDiscountPct, estimateLineProfitInfo, unitPriceForProduct } from "@/lib/pricing";
...
function calcUnit(product: Product, discountType: DiscountType, discountPct: number): number {
...
export function NewQuoteDialog({
    ...
    void (async () => {
      ...
      try {
        ...
        if (quoteId) {
          ...
          setDiscountType(q.discount_type === "none" ? "custom" : q.discount_type);
          setStatus(q.status);
          setNotes(q.notes || "");
          ...
          const loadedLines: Line[] = detail.items.map((item) => {
            ...
            return {
              ...
              quantity_raw: item.quantity_m2 == null ? "" : String(item.quantity_m2),
  ...
  }, [open, defaultCustomerId, quoteId, onOpenChange, defaultProductIds?.join(",")]);
  ...
  async function handleExportPdf() {
    ...
    try {
      ...
      if (!window.open(url, "_blank")) toast.error("Trình duyệt đang chặn cửa sổ xuất tài liệu");
  ...
  async function handleCopyLineCodes() {
    const codes = [
      ...new Set(
        lines.map((l) => (l.product_code || l.product?.code || "").trim()).filter(Boolean),
      ),
    ];
    if (!codes.length) {
        ...
        >
          <DialogHeader className="px-4 sm:px-6 pt-4 sm:pt-6 pb-2 flex-shrink-0">
            <div className="flex items-center justify-between gap-3 pr-8">
              <DialogTitle>
                {isEdit ? `Sửa báo giá${quoteCode ? ` ${quoteCode}` : ""}` : "Tạo báo giá"}
              </DialogTitle>
              {activeQuoteId ? (
          ...
          {loadingEdit ? (
            <div
              className="flex-1 min-h-0 animate-in fade-in duration-150 px-4 sm:px-6 py-4 space-y-4"
              aria-label="Đang tải báo giá"
            >
              ...
              <div className="flex-1 overflow-y-auto px-4 sm:px-6 space-y-4 pb-3">
                ...
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">
                      Sản phẩm ({lines.filter((l) => l.product).length}/{lines.length} dòng)
                    </span>
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        onClick={() =>
                          setLines((prev) => prev.map((line) => ({ ...line, collapsed: true })))
                        }
                      ...
                      <button
                        ...
                        onClick={() =>
                          setLines((prev) => prev.map((line) => ({ ...line, collapsed: false })))
                        }
                        className="size-8 grid place-items-center rounded-md text-muted-foreground hover:bg-surface-strong hover:text-foreground"
                      >
                            ...
                            <div className="flex-1 min-w-0">
                              <button
                                ...
                                className={`${inputCls} text-left flex items-center justify-between gap-2 ${
                                  !line.product ? "text-muted-foreground" : "text-foreground"
                                }`}
                                onClick={() => {
                                  if (locked) return;
                            ...
                            <div className="w-24 sm:w-32 flex-shrink-0">
                              <input
                                ...
                                onChange={(e) =>
                                  setLines((prev) =>
                                    prev.map((l) =>
                                      l.key === line.key ? { ...l, area: e.target.value } : l,
                                    ),
                                  )
                                }
                              />
                            ...
                            (line.product ? (
                              <>
                                            ...
                                            prev.map((l) =>
                                              l.key === line.key
                                                ? {
                                                    ...
                                                    packing: e.target.value.replace(/\D/g, ""),
                                                  }
                                                : l,
                                            ),
                                          )
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                      ...
                                      onBlur={() => {
                                        setLines((prev) =>
                                          prev.map((l) => {
                                            ...
                                            const qty = parseQuantityInput(l.quantity_raw);
                                            return {
                                              ...l,
                                              quantity_raw:
                                                l.quantity_raw.trim() === "" ? "" : String(qty),
                                              quantity_m2: qty,
                                            };
                                          }),
                                        );
                                  ...
                                  <label className="block">
                                    <span className="text-[10px] text-muted-foreground">CK %</span>
                                    <input
                                      type="number"
                                      min={0}
                                      max={100}
                                        ...
                                        setLines((prev) =>
                                          prev.map((l) => {
                                            ...
                                            return {
                                              ...l,
                                              unit_price: unitPrice,
                                              discount_pct: effectiveDiscountPct(retail, unitPrice),
                                            };
                                          }),
                                        );
                                      }}
                                  ...
                                  <label className="block">
                                    ...
                                    >
                                      {line.quantity_raw.trim() !== "" ? formatVND(lineTotal) : "—"}
                                    </div>
                                  </label>
                                </div>
                                ...
                                {profitInfo ? (
                                  ...
                                  >
                                    Lợi nhuận dự kiến: {formatVND(profitInfo.amount)} (
