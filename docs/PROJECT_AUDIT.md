...
|---|---|---|---|---|
| R1 | **Lỗi thật**: bắt chuỗi lỗi **SQLite** trong codebase **PostgreSQL** → nhánh bỏ qua dòng lỗi không bao giờ chạy, 1 `product_code` sai làm hỏng cả lần import | `src/api/functions.ts:1574` (`"FOREIGN KEY constraint failed"`) vs `src/db/driver.ts:1` (`pg`) | **CAO** | Chỉ báo cáo — sửa sẽ **đổi hành vi** import |
...
| R4 | **Route khổng lồ**: 3 file 2.3k–2.7k dòng chứa facet, sort codec, phân trang, token hoá tìm kiếm | `_app.thu-vien.tsx` (2699), `_app.luu-tru.tsx` (2386), `_app.san-pham.tsx` (2322) | **CAO** | Báo cáo (NEEDS_REVIEW) — tách hook là refactor lớn |
| R5 | **29 lỗi TypeScript tồn đọng**, trong đó `_app.luu-tru.tsx` thiếu hẳn import 3 type (`ImageRoomTagSlug`, `ProductImageKind`, `ProductImageRow`) | `npx tsc --noEmit` → 29 lỗi | Trung bình | Báo cáo |
...
| R7 | **`src/lib/storage.ts` phá quy ước đặt tên**: import `node:fs`, ghi `public/images` nhưng không có hậu tố `.server.ts` → lưới an toàn import-protection không phủ | `src/lib/storage.ts:1` vs `image-upload.server.ts`, `brand-assets.server.ts` | Trung bình | ✅ Đổi tên trong cleanup |
