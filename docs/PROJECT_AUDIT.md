...
| R7 | **Lưới import-protection vô hiệu** (nặng hơn tên file): `vite.config.ts` ghi đè `client.files` bằng `["**/server/**"]`, trong khi repo **không có thư mục `server/`** nào → luật mặc định của framework (`**/*.server.*`) bị thay thế và mất tác dụng. Hệ quả: `src/lib/storage.ts` (import `node:fs`, ghi `public/images`) không có hậu tố `.server.ts` và cũng **không** bị chặn | `vite.config.ts:38-44` (cũ) vs `defaults.js` của `@tanstack/start-plugin-core` (`files: ["**/*.server.*"]`) | Trung bình | ✅ Sửa trong cleanup: thêm `**/*.server.ts` vào `client.files` + đổi tên `storage.ts` → `storage.server.ts`. **Đã kiểm chứng bằng test âm** — build FAIL đúng như mong đợi khi client import `.server.ts` |
